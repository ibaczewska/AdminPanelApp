 <template>
    <div class="datetime-picker" :style="{ width: width }">
        <input type="text" :name="name" :readonly="readonly" :value="value" @click="show = !show">
            ...
            <table class="date-picker">
                ...
                <tbody>
                    <tr v-for="i in 6" :key=i>
                        <td v-for="j in 7" :key=j :class="date[i * 7 + j] && date[i * 7 + j].status" :date="date[i * 7 + j] && date[i * 7 + j].date" @click="pickDate(i * 7 + j)">{{date[i * 7 + j] && date[i * 7 + j].text}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
